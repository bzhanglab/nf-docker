...
write.table(round(log2(raw_matrix+1),2),file=file.path(output_dir, paste0(matrix_name_flag,"_raw_log2(x+1)_BCM.txt")),quote = FALSE,sep="\t")
write.table(round(log2(normalized_matrix+1),2),file=file.path(output_dir, paste0(matrix_name_flag,"_UQ_log2(x+1)_BCM.txt")),quote = FALSE,sep="\t")

pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_raw_log2(x+1)_BCM_boxplot.pdf")),height=7,width=12)
boxplot(log2(raw_matrix+1),las = 2,ylab="Log2(RSEM+1)",main="Distribution of gene expression")
dev.off()
pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_UQ_log2(x+1)_BCM_boxplot.pdf")),height=7,width=12)
...
pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_raw_log2(x+1)_BCM_corplot.pdf")),height=7,width=7)
...
pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_UQ_log2(x+1)_BCM_corplot.pdf")),height=7,width=7)
corrplot(M, method = "color",order = "hclust",tl.cex = 0.3,cl.lim=c(min(M),max(M)), is.corr=FALSE)
dev.off()

if(sum(grepl("_A",colnames(raw_matrix)))>0&sum(grepl("_T",colnames(raw_matrix)))>0)
{
	pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_raw_log2(x+1)_BCM_pcaplot.pdf")),width=6,height=4)
	sample_labels=rep("Tumor",length(colnames(raw_matrix)))
	sample_labels[grepl("_A",colnames(raw_matrix))]="Normal"
	res.pca <- prcomp(na.omit(t(raw_matrix)),scale = FALSE)
	p <- fviz_pca_ind(res.pca, label="none", habillage=sample_labels,addEllipses=TRUE, ellipse.level=0.95)
	print(p)
	dev.off()
   
   pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_UQ_log2(x+1)_BCM_pcaplot.pdf")),height=4,width=6)
   sample_labels=rep("Tumor",length(colnames(raw_matrix)))
   sample_labels[grepl("_A",colnames(raw_matrix))]="Normal"
   res.pca <- prcomp(na.omit(t(raw_matrix)),scale = FALSE)
   p <- fviz_pca_ind(res.pca, label="none", habillage=sample_labels,addEllipses=TRUE, ellipse.level=0.95)
   ...
   pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_raw_log2(x+1)_BCM_pcaplot.pdf")),width=6,height=4)
	...
	pdf(file=file.path(output_dir, paste0(matrix_name_flag,"_UQ_log2(x+1)_BCM_pcaplot.pdf")),width=6,height=4)
	res.pca <- prcomp(na.omit(t(raw_matrix)),scale = FALSE)
	p <- fviz_pca_ind(res.pca, label="none")
	print(p)
	dev.off()	
...
linear_gene_expression=read.table(file.path(output_dir, "gene_fpkm_removed_circRNA_raw_log2(x+1)_BCM.txt"),header=TRUE,row.names=1,check.name=FALSE,sep="\t")
