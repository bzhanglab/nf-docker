#  https://raw.githubusercontent.com/bzhanglab/neoflow/master/bin/binding_prediction.py
import argparse
import csv
import os
import re
...
tmp_dir = 'tmp_' + rnd_str 
os.makedirs(os.path.join(save_path, tmp_dir))
...
with open(AA_form_file) as csvfile2:
    ...
    with open(save_path + tmp_dir + '/somatic_mutation_reference.csv', "a") as csvfile1:
...
fasta_file = save_path + tmp_dir + '/neoantigen_candidates.fasta'
...
def typing(line,netMHCpan,out_dir):
    ...
    a2 = '-a ' + HLA_type_long
    a3 = '-l 8,9,10,11'
    a4 = '>' + out_dir + tmp_dir + '/' + HLA_type_short + "_netMHCpan.csv"
...
typing_files = []
for x in os.listdir(save_path + tmp_dir):
    if 'netMHCpan' in x: typing_files.append(x)

save_name = save_path + tmp_dir + '/netMHCpan_binding_result.csv'

for raw_path in typing_files:
    HLA_type = raw_path.split('_')[0]
    HLA_type = HLA_type[:5] + '*' + HLA_type[5:7] + ':' + HLA_type[7:]
    raw_data = []
    with open(save_path + tmp_dir + '/' + raw_path) as csvfile3:
...
if (len(records) != 0):
    binding_result = pd.read_csv(save_name,
                             ...
                             names=['HLA_type',
                                    ...
                                    'netMHCpan_precentile_rank'], low_memory=False)
    somatic_mutation = pd.read_csv(save_path + tmp_dir + '/somatic_mutation_reference.csv', low_memory=False)
    ...
    empty_writer.write("Variant_ID,Chr,Start,End,Ref,Alt,Variant_Type,Variant_Function,Gene,mRNA,Neoepitope,Variant_Start,Variant_End,AA_before,AA_after,HLA_type,netMHCpan_binding_affinity_nM,netMHCpan_precentile_rank")

# clean up
os.system('rm -r ' + save_path + tmp_dir)
